...
def retrieveResiduals(args):
    ...
    # If SV input, extract epoch and state from the dataframe row
        ...
        line2 = row["epoch"]
        line1 = np.array([row["xpos"], row["ypos"], row["zpos"], 
                      row["xvel"], row["yvel"], row["zvel"]])
    ...
    for j, row2 in enumerate(obs):
        ...
        res_temp = abs(np.arccos(np.sin(delta_obs)*np.sin(delta_obs_est) + np.cos(delta_obs)*np.cos(delta_obs_est)*np.cos(alpha_obs - alpha_obs_est)))
...
def retrieveResidualsTLE(args):
    ...
    for j, row2 in enumerate(obs):
        ...
        # Stack data onto array
        newRow = np.array([resultsDict['semiMajorAxisKmDif'],resultsDict['eccentricityDif'],resultsDict['inclinationDegDif'],resultsDict['raanDegDif'],resultsDict['argPerigeeDegDif'],resultsDict['meanAnomalyDegDif']])
    ...
    orbital_elements = ['semiMajorAxis_km', 'eccentricity', 'inclination_deg',
                        'raan_deg', 'argPerigee_deg', 'meanAnomaly_deg']
