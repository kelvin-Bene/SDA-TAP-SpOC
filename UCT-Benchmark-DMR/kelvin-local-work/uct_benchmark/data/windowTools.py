...
class DatasetCode:
    ...
    # UDL service names
    sensor_type_queries = {"OP": "eoobservation", "RA": "radarobservation", "RF": "rfobservation"}
    ...
    def __lt__(self, other):
        if (
            (self.ObjType == other.ObjType or self.ObjType == "U")
            ...
            and (
                ...
                or other.SensorType == "FU"
                or other.SensorType in self.sensor_superiors[self.SensorType]
            )
            and (
                self.coverage_hierarchy[self.PercentOrb]
                <= self.coverage_hierarchy[other.PercentOrb]
            )
            and (
                self.track_gap_hierarchy[self.TrackGapPer]
                <= self.track_gap_hierarchy[other.TrackGapPer]
            )
            and (
                self.obs_count_hierarchy[self.ObsCount] <= self.obs_count_hierarchy[other.ObsCount]
            )
            and (
                self.obj_count_hierarchy[self.ObjCount] <= self.obj_count_hierarchy[other.ObjCount]
            )
    ...
    def __str__(self):
        return (
            f"{self.ObjType}{self.ObjDist}{self.Regime}{self.Event}{self.SensorType}"
...
def createDatasetCodeGUI():
    ...
    percent_orbit = {"All Low Coverage": 0, "Some Low Coverage": 0, "No Low Coverage": 1}
    ...
    observation_count = {"All Low Obs Count": 0, "Some Low Obs Count": 0, "No Low Obs Count": 1}

    total_object_count = {
        "High Object Count": 0,
        "Standard Object Count": 0,
        "Low Object Count": 1,
    }
    ...
    class ParameterEditor(ctk.CTk):
        def __init__(self):
            ...
            for group_name, group_data in dataset_parameters.items():
                ...
                label = ctk.CTkLabel(
                    self.scroll_frame, text=group_name, font=ctk.CTkFont(weight="bold", size=16)
                )
            ...
            save_button = ctk.CTkButton(
                self.scroll_frame, text="Save Changes", command=self.save_parameters
            )
...
def codeGenerator(parameters: pd.DataFrame) -> list[DatasetCode]:
    """
    ...
    # --- Target Object Types ---
    target_mask = []
    targ_ob_dict = parameters.at[
        0, "Value"
    ]  # Dictionary of object type flags (e.g., {'H': True, 'C': False, ...})
    ...
    obj_dist_comb = [
        str(x) for x in parameters.at[1, "Value"]
    ]  # List of object distribution codes
    print(obj_dist_comb)
...
def setRecycler(desired_sets, existing_sets):
    ...
    for s in np.unique(scores_sorted[:, 1]):
        ...
        desired_DatasetCodes = [
            code for code in desired_DatasetCodes if code not in unrecyclable_sets
        ]
