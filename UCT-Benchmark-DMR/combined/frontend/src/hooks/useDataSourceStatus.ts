...
export function useDataSourceStatus() {
  ...
  const getMissingForPreset = (presetId: string): string[] => {
    ...
    return required.filter((svc) => {
      ...
      return !cred?.is_configured && !cred?.has_env_fallback;
    });
  };
  ...
  const sources: DataSourceStatus[] = (credentials || []).map((cred: CredentialServiceInfo) => ({
    ...
    isConfigured: cred.is_configured || cred.has_env_fallback,
   ...
   */
  const udlCred = credentials?.find((c) => c.service_name === 'udl');
  const isUdlConfigured = (udlCred?.is_configured || udlCred?.has_env_fallback) ?? false;
