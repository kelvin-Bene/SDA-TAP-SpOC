...
import { useUCTPPModels, useTrainUCTPPModel } from '@/hooks/useUCTPPModels';
...
import type { UCTPPModelType } from '@/types/uctp';
...
export function MLTrainingPage() {
  const { data: models } = useUCTPPModels();
  const trainModel = useTrainUCTPPModel();
  const [showForm, setShowForm] = useState(false);
  const [name, setName] = useState('');
  const [modelType, setModelType] = useState<UCTPPModelType>('clustering_nn');
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-2 max-w-2xl">
              Train machine learning models to enhance UCTP processing accuracy for clustering, propagation, and orbit refinement.
            </p>
          </div>
          <Button
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    value={modelType}
                    onChange={(e) => setModelType(e.target.value as UCTPPModelType)}
                    className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm focus:border-stellar-purple/50 focus:outline-none focus:ring-1 focus:ring-stellar-purple/20 transition-all"
                  >
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                {(Object.entries(MODEL_TYPE_META) as [UCTPPModelType, typeof MODEL_TYPE_META[UCTPPModelType]][]).map(
                  ([key, meta]) => {
        ...
        {models && models.length > 0 ? (
          ...
          <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.01] p-12 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1 max-w-sm mx-auto">
              Train your first ML model to enhance UCTP processing accuracy. Click "Train New Model" above to get started.
            </p>
          </div>
        )}
      </div>
