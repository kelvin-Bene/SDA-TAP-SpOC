...
from uct_benchmark.settings import RAW_DATA_DIR
import uct_benchmark.api.apiIntegration as UDL
import uct_benchmark.settings as con
...
def batchPull(code, start_epoch, end_epoch, UDL_token):
    ...
    logger.info(f"Batch Size (days): {batchsize.days}")

    # print sensortypes being queried
    sensor_types = [
        key
        for key, value_list in code.sensor_superiors.items()
        if code.SensorType in value_list or code.SensorType == key
    ]
    logger.info(f"Sensor Types being queried: {sensor_types}")
    ...
    logger.info(f"Orbital Regimes being queried: {regimes}")

    # read time winow
    time_window = int(code.TimeWindow)
    logger.info(f"Time Window being queried (days): {time_window}")
    ...
    for sensor_type in sensor_types:
        logger.info(f"Querying sensor type: {sensor_type}")
        service = code.sensor_type_queries[sensor_type]
        if isinstance(regimes, str):
            regimes = [regimes]

        for regime in regimes:
            logger.info(f"Querying regime: {regime}")
            param_dict = {"range": code.regime_ranges[regime], "uct": "false", "dataMode": "REAL"}

            steps = dt.timedelta(minutes=0)
            while steps < batchsize:
                logger.info(f"Querying time chunk starting at: {start_time + steps}")
                # run query command
                final_dict = param_dict.copy()
                final_dict["obTime"] = (
                    UDL.datetimeToUDL(start_time + steps)
                    + ".."
                    + UDL.datetimeToUDL(start_time + dt.timedelta(minutes=10) + steps)
                )
                steps += dt.timedelta(minutes=10)
                data = UDL.UDLQuery(UDL_token, service, final_dict)
                if not data.empty:
                    logger.info("Appending data chunk...")
                    data_list.append(data)
                else:
                    logger.info("No data chunk returned...")

    logger.info("Compiling batch data...")
...
def thresholdConvert(thresholds_strs):
    ...
    tier_to_value = {"T1": 4, "T2": 3, "T3": 2, "T4": 1, "T5": 0}
...
def thresholdCheck(batch_temp, thresh_des, code):
    ...
    thresh_curr, _, _ = basicScoring(
        code, batch_temp, satData=pd.read_csv((RAW_DATA_DIR / "satelliteData_Full.csv"))
    )
...
def slide(sub_batch, window_size, code):
    ...
    while i >= 0 and tc + window_size <= t_max:
        ...
        bin = sub_batch[
            (sub_batch["epochNormalized"] >= t0) & (sub_batch["epochNormalized"] <= tf)
        ]
        ...
        # Score the bin
        score_temp, orbElems_temp, metadata_temp = basicScoring(
            code, bin, satData=pd.read_csv((RAW_DATA_DIR / "satelliteData_Full.csv"))
        )
    ...
    # Pull the bin
    bin = sub_batch[
        (sub_batch["epochNormalized"] >= t0_forced) & (sub_batch["epochNormalized"] <= t_max)
    ]
...
def bisect(batch, window_size, thresh_des, code):
    ...
    def recurse(sub_batch):
        ...
        # Bin is valid but is minimum size and must be swept
            ...
            bin_temp, score_temp, orbElems_temp, metadata_temp = slide(
                sub_batch, window_size, code
            )
...
def windowCheck(window_size, batch_size, code, start_epoch, end_epoch, UDL_token):
    ...
    logger.info(f"Thresholds to meet: {threshold_scores}")
    ...
    for i in range(len(threshold_scores)):
        ...
        # -------------------------------#
        logger.info(f"Starting batch iteration {i+1} for threshold {thresh_des}...")
        # -------------------------------#
        ...
        print(
            f"Batch Size: {data_sorted['epochNormalized'].max() - data_sorted['epochNormalized'].min()}"
        )
        # -------------------------------#
        ...
        if thresholdCheck(data_sorted, thresh_des, code):
            ...
            bin_temp, score_temp, orbElems_temp, metadata_temp = bisect(
                data_sorted, window_size, thresh_des, code
            )
        ...
        batch_size_current = max(
            expFunc(window_size, batch_size, i + 1, con.batchSizeDecayRate), 2.01 * window_size
        )
...
def windowMain(codes, UDL_token):
    ...
    for code in codes:
        # -------------------------------#
        logger.info(f"Started Code: {str(code)}")
        # -------------------------------#
        ...
        logger.info("Starting window check...")
        bin_best, score, orbElems, metadata = windowCheck(
            window_size, initial_batch_size, code, start_epoch, end_epoch, UDL_token
        )
