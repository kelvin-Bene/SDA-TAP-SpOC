...
class EnhancedDatasetCode:
    ...
    def from_code(cls, code: str) -> 'EnhancedDatasetCode':
        ...
        enhanced_pattern = r'^([A-Z]{4})_([A-Z]{3})_([A-Z]{3})_([A-Z]{2})_([A-Z0-9]{3})_(\d{2})D_(\d{3})$'
...
def verify_reproducibility(
    config: DatasetConfig,
    previous_metadata_path: Union[str, Path]
    ...
    if config.duration_days != prev_config.get('duration_days'):
        differences.append(f"duration_days: {prev_config.get('duration_days')} -> {config.duration_days}")
