...
async def get_dataset(
    ...
    # Get satellite NORAD IDs using multiple fallback strategies
    satellites = []

    # Strategy 1: dataset_references table
    try:
        sat_result = db.execute(
            "SELECT sat_no FROM dataset_references WHERE dataset_id = ? ORDER BY sat_no",
            (id_int,),
        )
        satellites = [row[0] for row in sat_result.fetchall()]
    except Exception:
        pass

    # Strategy 2: JOIN observations via dataset_observations links
    if not satellites:
        try:
            sat_result2 = db.execute(
                """
                SELECT DISTINCT o.sat_no
                FROM observations o
                JOIN dataset_observations dso ON o.id = dso.observation_id
                WHERE dso.dataset_id = ? AND o.sat_no IS NOT NULL
                ORDER BY o.sat_no
                """,
                (id_int,),
            )
            satellites = [row[0] for row in sat_result2.fetchall()]
        except Exception:
            pass

    # Strategy 3: Extract from generation_params (covers case where observations
    # were persisted to a different database file)
    if not satellites and params:
        # Try actual_satellites first (satellites that had data), then requested list
        sat_ids_from_params = (
            params.get("actual_satellites")
            or params.get("satellites")
            or params.get("satIDs")
            or []
        )
        if sat_ids_from_params:
            satellites = [int(s) for s in sat_ids_from_params]
...
@router.patch("/{dataset_id}/params")
async def update_dataset_params(
    dataset_id: str,
    request: Request,
    db: DatabaseManager = Depends(get_db),
):
    """Update a dataset's generation_params (merge new keys into existing params)."""
    id_int = validate_dataset_id(dataset_id)
    body = await request.json()

    result = db.execute("SELECT id, generation_params FROM datasets WHERE id = ?", (id_int,))
    row = result.fetchone()
    if row is None:
        raise HTTPException(status_code=404, detail="Dataset not found")
    ...
    db.execute(
        ...
        (json.dumps(existing), id_int),
    )
    return {"message": f"Dataset {dataset_id} params updated", "params": existing}
