...
async def get_sample_submission(
    ...
    Returns a downloadable JSON file with synthetic UCTP results at the
    specified quality level. Observation IDs in `sourcedData` reference
    real observations in the selected dataset so the evaluation flow works
    end-to-end. Demo-mode only — returns 404 otherwise.
    ...
    # Stable mapping so downloads are deterministic across process restarts
    quality_seed = {"high": 1, "medium": 2, "low": 3}.get(quality, 2)
    rng = _random.Random(id_int * 10 + quality_seed)
