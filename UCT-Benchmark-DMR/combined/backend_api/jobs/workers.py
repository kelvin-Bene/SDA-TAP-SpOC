...
def run_dataset_generation(
    ...
    try:
        ...
        # Save the satellite list to config so it persists in generation_params
        ...
        # Pass the backend's database path so observations go to the correct DB
        from backend_api.database import get_database_path
        backend_db_path = str(get_database_path())
        ...
        # Save actual satellites that had data to generation_params
        if actual_sats is not None:
            config["actual_satellites"] = [int(s) for s in actual_sats]
        ...
        # Update the dataset status with all metrics and generation_params
        db.execute(
            """
            UPDATE datasets
            ...
            WHERE id = ?
            """,
            (observation_count, satellite_count, avg_coverage, json.dumps(_convert_numpy_to_native(config)), dataset_id),
        )
