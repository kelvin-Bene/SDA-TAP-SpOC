...
def pull_observations(target_obs: int, days: int, sats_per_regime: int) -> Tuple[pd.DataFrame, pd.DataFrame, Dict]:
    ...
    if not token:
        ...
        if not username or not password:
            raise ValueError("UDL credentials not found in .env (need UDL_TOKEN or UDL_USERNAME/UDL_PASSWORD)")
    ...
    for regime, config in ORBITAL_REGIMES.items():
        ...
        try:
            # Get satellites
            sat_df = UDLQuery(token, "elset/current", {
                "period": f"{period_min}..{period_max}",
                "maxResults": sats_per_regime * 2
            })

            if sat_df is None or len(sat_df) == 0:
                print(f"  No satellites found")
                continue
            ...
            # Get observations
            obs_params = [{"satNo": str(s), "obTime": sweep_time, "dataMode": "REAL", "maxResults": 5000} for s in sat_ids]
            ...
            if obs_df is not None and len(obs_df) > 0:
                ...
                for sid in obs_df['satNo'].unique():
                    sat_info[int(sid)] = {'regime': regime, 'obs_count': len(obs_df[obs_df['satNo'] == sid])}

                print(f"  Retrieved {len(obs_df):,} observations from {obs_df['satNo'].nunique()} satellites")

                # Get TLEs using elset/current
                tle_sats = obs_df['satNo'].unique()
                for sid in tle_sats:
                    try:
                        tle_result = UDLQuery(token, "elset/current", {"satNo": str(sid), "maxResults": 1})
...
def test_orekit_simulation(obs_df: pd.DataFrame, tle_df: pd.DataFrame, results_dir: Path) -> Dict:
    ...
    log_subsection("Summary")
    print(f"TLE: {results['tle']['passed']} passed, {results['tle']['failed']} failed, {results['tle']['obs']} obs")
    print(f"SV: {results['sv']['passed']} passed, {results['sv']['failed']} failed, {results['sv']['obs']} obs")
...
def generate_report(results_dir: Path, pull: Dict, ds: Dict, gap: Dict, sim: Dict) -> Dict:
    ...
    overall = pull['target_met'] and ds_rate >= 0.6 and gap_rate >= 0.8 and tle_rate >= 0.8 and sv_rate >= 0.8
...
def main():
    ...
    try:
        # Phase 1: Data Pull
        obs_df, tle_df, pull_meta = pull_observations(args.target_obs, args.days, args.sats_per_regime)
        ...
        print(f"Error: {e}")
        traceback.print_exc()
        save_json({'error': str(e), 'traceback': traceback.format_exc()},
