...
The UCT Benchmark application captures approximately **~87%** of the original vision described by Louis Caves and the AFRL Scholars team (up from 75-80% at the Apr 2 baseline). The core evaluation pipeline (orbit association, binary metrics, state metrics, residual metrics) is **substantially implemented and algorithm-correct**. The dataset generation pipeline is largely implemented with all key algorithms present. The web platform (UI, authentication, leaderboard, 3D globe) goes **beyond** what was originally requested and represents a significant value-add.

**What closed between Apr 2 and Apr 22:** (1) composite scoring implemented and surfaced in the UI (commit `f2fdd50`); (2) **3D orbit globe fully working in prod** with owner-gated visibility — took a 5-commit fix train to land (JVM eager warm at FastAPI startup, propagator import-order fix so `initVM()` runs before `pyhelpers` imports `java.io.File` at module load, nginx proxy switched to public backend URL after Railway internal DNS failed, backend `Cache-Control: no-store` on `/api/*` to stop browsers from permanently caching transient 410s, CORS `allow_headers` extended for `Cache-Control`); (3) Orekit library available in production for both evaluation workers AND web request threads (commits `51b56ba` / `e959d09` / `d9c38c3`); (4) **first completed end-to-end submission in prod history** on 2026-04-17 — submission 44 against dataset 158, composite_score=1.0; (5) submission 47 (DemoAlgoFresh) achieves F1=1.0 with full state + residual metrics; (6) `/predictions` endpoint gracefully serves reference orbits even after UCTP files are cleaned from storage (commit `d81e150`), so the Results page Orbits tab renders for historical submissions.

**Known gaps as of 2026-04-22:** (a) event data sourcing — the ML event-labeling team output was never integrated (original Benchmarking Doc acknowledged "the ML Model is not operating"); (b) non-optical sensor integration (Louis acknowledged optical-only plan in Jan 22 transcript). Both remain external-dependency-bound.
...
7. ~~**End-to-End Evaluation in Production**~~: **RESOLVED (2026-04-22)**. Evaluation + globe both verified on live prod. Eval pipeline: `scripts/seed_demo_submission.py` (commit `0879bdd`) produced `composite_score=1.0` on submission 44 against dataset 158 with full binary + state + residual metrics. Globe: `/reference-orbits` endpoint initially failed with `Propagation failed for sat N: Attempt to create Java package 'java' without jvm` — root cause was `orekit_jpype.pyhelpers` being imported before `orekit.initVM()` in `ephemerisPropagator` and `monteCarloPropagator` (TLEpropagator had the correct order, which is why eval workers happened to work — they hit TLEpropagator first). Fixed across commits `d81e150` (eager warm_jvm at FastAPI startup + graceful /predictions when UCTP file gone), `189695f` (import order in propagator.py), `7698337` (nginx proxy → public backend URL, bypasses flaky Railway internal DNS), `5eb6f0b` (backend Cache-Control: no-store on /api/* + frontend axios Cache-Control: no-cache to bust stuck browser 410s), `f817ea8` (CORS allow-headers for Cache-Control). Browser verification: `.cesium-widget canvas` 1101×499 mounts for owner on `/results/47` showing "Showing 1 satellites".
...
| **External Integrations** | 92% | UDL, ESA, Space-Track all working. Orekit library available in production for both eval workers AND web request threads (JVM eagerly warmed at FastAPI startup + propagator import-order fix). ML labeling still absent (external blocker). |
...
**Overall Alignment: ~87%** *(promoted 2026-04-22 after Claude-in-Chrome verification: Cesium canvas mounts with real satellite tracks on `/results/47`, `/reference-orbits/153` returns populated satellite data, /predictions gracefully serves reference even when UCTP is gone.)*
...
2. ~~**Ensure Orekit Availability in Production**~~: **RESOLVED (2026-04-22)**. `orekit-data.zip` ships in the backend Docker image (commits `51b56ba` / `e959d09` / `d9c38c3`). Production `/health` reports `orekit: available`. JVM is now eagerly initialized at FastAPI startup (commit `d81e150`) with the import order in `ephemerisPropagator`/`monteCarloPropagator` fixed so `orekit.initVM()` runs before `from orekit_jpype.pyhelpers import setup_orekit_curdir` (commit `189695f`). All three evaluation categories (binary, state, residual) AND the web-path `/reference-orbits` endpoint verified working in prod.
...
5. ~~**Verify End-to-End Pipeline**~~: **CONFIRMED (2026-04-22)**. Submission 44 against dataset 158 ran generate → download → submit → evaluate → view results → leaderboard with `composite_score=1.0`. Submission 47 (DemoAlgoFresh) achieves F1=1.0/precision=100%/recall=100% with position RMS 232.70 km. 86 of 92 prod Playwright specs pass on the live Railway deploy (6 skip gracefully where data fixtures don't exist — the Cesium canvas-mount spec now skips with a clear message when no owned dataset has real reference orbits, rather than falsely failing). Visual confirmation via Claude-in-Chrome: `/results/47` Orbits tab → Reference → `.cesium-widget canvas` 1101×499 renders 1 MEO satellite track. **PDF report generation is also shipped and verified** — `GET /api/v1/results/{id}/report?format=pdf` (at `backend_api/routers/results.py:465-632`) calls `evaluationReport.generate_pdf_report` (the 998-LOC module at `uct_benchmark/evaluation/evaluationReport.py`, not the legacy `utils/generatePDF.py`); the "Download Report" button on `ResultsPage.tsx:218` is wired via `useDownloadReport` hook. Tested 2026-04-22 against submissions 47 and 44: both return HTTP 200 with `Content-Type: application/pdf`, 41530 bytes, valid `%PDF-1.4` header. (Prior audit revisions listing PDF as "residual work" were stale — this has been working since v2.0.0.)
