...
**Known gaps as of 2026-04-22:** (a) event data sourcing — the ML event-labeling team output was never integrated (original Benchmarking Doc acknowledged "the ML Model is not operating"); (b) PDF report generation in the web flow (backend `generatePDF.py` exists but isn't wired into a user-facing download button); (c) non-optical sensor integration (Louis acknowledged optical-only plan in Jan 22 transcript). (a) and (c) remain external-dependency-bound; (b) is a small wire-up task.
