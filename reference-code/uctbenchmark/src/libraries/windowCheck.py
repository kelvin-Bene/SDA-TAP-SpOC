...
def batchPull(code, start_epoch, end_epoch, udl_token):
    ...
    with open(f'{str(code)}_{str(sensor_type)}_{str(start_epoch)}_{str(end_epoch)}.json', 'a') as data_file:
...
def windowMain(codes, udl_token, raw_dir):
    ...
    for code in codes:
        ...
        # Creating directory for UDL response under search code
        dir_name = f"{dt.datetime.now()}_0.0.3_{code}"
