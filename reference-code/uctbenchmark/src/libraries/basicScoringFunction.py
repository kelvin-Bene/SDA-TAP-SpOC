...
sample_window = os.getenv("SAMPLE_WINDOW")
...
from uctbenchmark.src.libraries.orbitCoverage import orbitCoverage
from uctbenchmark.src.libraries.propagator import orbit2OE, datetime2AbsDate
...
def basicScoring(datasetCode, allObs, satData):
    ...
    timeSpan = (pd.to_datetime(allObs['obTime']).max() - pd.to_datetime(allObs['obTime']).min()).total_seconds()/86400
    ...
    for regime, thresh in thresholds.items():
        ...
        lowCovgSats += results_df[mask & (results_df['orbitCoverage'] < thresh) & (results_df['orbitCoverage'] >= tooLow)].satNo.tolist()
    ...
    targetObjects = partNumber.ObjType
    targetObjPerc = int(partNumber.ObjDist)
    orbitRegime = partNumber.Regime
    eventClass = partNumber.Event
    sensorType = partNumber.SensorType
    ...
    fitSpan = int(partNumber.TimeWindow)
    ...
    if targetObsCount == 'A':
        ...
        if not countGood:
            T2Obs = ((lowRatio < stdPercentage) or ((stdRatio < stdPercentage) and highRatio > stdPercentage)) and enoughObj
            if T2Obs:
    ...
    flags = {
        'T1': T1,
        'T2': T2covg or T2Gap or T2Obs,
        'T3': T3covg or T3Gap or T3Obs,
        'T4': T4
    }
    ...
    manipulationRequired = {
        'Obs Count': {'Tier': flagObsCount, 'Number of Downsamples from High obs to std obs': (dwnObsStd),
                     'Number of downsamples from std to low':  (dwnObsLow), 'Number of obs simf from low to std': (simObs),
                     'List of satellites with Low Obs count': lowObsSats, 'List of satellites with Standard Obs Count': stdObsSats, 'List of satellites with high obs count': highObsSats},
        'Orbit Coverage': {'Tier': flagCoverage, 'Number of downsamples required': (dwnCovg), 'Number of simulations required': (simCovg),
                           'List of satellites with low coverage': lowCovgSats, 'List of satellites with standard coverage': stdCovgSats},
        'Track Gap': {'Tier': flagTrackGap, 'Number of downsamples required': (dwnGap), 'Number of sims required': (simGap),
                      'List of satellites with long track gap': longGapSats, 'List of satellites with standard track gap': stdGapSats}
    }
...
if __name__ == '__main__':
    ...
    from libraries.windowTools import DatasetCode
    partNumber = 'H50GEONEOPSSSS03'
    datasetcode = DatasetCode(partNumber)
    satData = pd.read_csv(sat_data_f)
    allObs = pd.read_csv(sample_window)
