...
setup_orekit_data(filenames=os.path.join(os.getenv("EXT_DIR"), "orekit-data"), from_pip_library=False) #Loading orekit data
...
def loadDataset(input_path):
    ...
    ref_sv['cov_matrix'] = ref_sv['cov'].apply(lambda x: np.array(json.loads(x)))
