...
def binTracks(ref_obs, ref_sv, cutoff=90):
    ...
    ref_obs_wip['grp_key'] = np.where(
        ...
        ref_obs_wip['senlat'].astype(str) + '_' +
        ref_obs_wip['senlon'].astype(str) + '_' +
        ref_obs_wip['senalt'].astype(str) + '_' +
    ...
    # --------------------------------------------------------------------
    ref_obs.drop(columns=[col for col in added_cols if col in ref_obs.columns], inplace=True, errors='ignore')
...
def _downsampleAbsolute(ref_obs, sat_params, objp, obs_max, rand, rng, chosen_sats=None, bins=10):
    ...
    orbital_periods = {sat: elems['Period'] for sat, elems in sat_params.items() if np.isfinite(elems['Period'])}
    ...
    sat_obs_counts = {sat: elems['Number of Obs'] for sat, elems in sat_params.items() if np.isfinite(elems['Number of Obs'])}
    ...
    remaining_sats_sorted = sorted(
        remaining_sats,
        key=lambda s: sat_obs_counts[s]
    )
    ...
    for sat, sat_df in grouped_obs.items():
        if sat in sampled_satID_set and len(sat_df) > obs_max:
            ...
            target_per_bin = (
                pd.Series(base_per_bin, index=bin_counts.index)
                + pd.Series([1] * remainder + [0] * (n_bins - remainder), index=bin_counts.index)
            )
            ...
            sampled = (
                time_sorted
                .groupby('time_bin', group_keys=False, observed=True)
...
def downsampleData(ref_obs, sat_params, orbit_coverage, track_length, obs_count, bins=10, rand=None):
    ...
    # Coverage
    ref_obs, _ = _lowerOrbitCoverage(ref_obs, sat_params, orbit_coverage['p_bounds'], orbit_coverage['p_coverage'], rng, orbit_coverage['sats'])
    # Gaps
    ref_obs, _ = _increaseTrackDistance(ref_obs, sat_params, track_length['p_bounds'], track_length['p_track'], rng, track_length['sats'])
    # Max count
    ref_obs = _downsampleAbsolute(ref_obs, sat_params, obs_count['p_bounds'], obs_count['obs_max'], rand, rng, obs_count['sats'])
    ...
    def _filter_sats(candidate_list, valid_sats):
        return set(valid_sats) if candidate_list is None else set(valid_sats).intersection(candidate_list)
    ...
    cov = sum(
        v <= orbit_coverage['p_coverage']
        for sat, v in coverages.items()
        if sat in cov_sats
    )
    ...
    periods = pd.Series(
        {sat: elems['Period'] for sat, elems in sat_params.items()}
    ).reindex(max_gaps.index)
    ...
    # Find current counts
    count = len([
        sat for sat in cnt_sats
        if len(grouped_obs.get(sat, [])) <= obs_count['obs_max']
    ])
