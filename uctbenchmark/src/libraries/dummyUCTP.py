...
def dummy(truth_data_filename, blind_data_filename): 
    ...
    index_key = {value: idx for idx, value in enumerate(satList)}

    fake_obs_corrs = pd.DataFrame({'id': obsSatDF['id'].values})

    # Generate random probabilities
    random_vals = np.random.random(len(fake_obs_corrs)) # generates list of random of len fake_obs_corrs
    ...
    with open(uctp_data, "w") as json_out:
        to_json = []
