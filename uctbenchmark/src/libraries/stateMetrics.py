...
from uctbenchmark.src.libraries.apiIntegration import TLEToSV
...
def _propRef(ref_orbits, cand_orbits, propagator):
    ...
    args_list = [
        (referenceStates[j], referenceCovs[j], referenceEpochs[j], candidateEpochs[j],
         [mass[j], area[j], drag[j], solar[j]], propagator)
...
def stateMetrics(ref, candidate, propagator, elset_mode=False):
    ...
    if elset_mode:   
        ...
        cand_states = [TLEToSV(l1,l2) for l1, l2 in zip(candidate['line1'].tolist(), candidate['line2'].tolist())]
