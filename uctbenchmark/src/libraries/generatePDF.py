...
def generatePDF(data, eval_pdf, path_flag = False):
    if path_flag:
    ...
    class MyPDF(FPDF):
        def header(self):
            ...
            self.image(f"{figs}/Reverse_IH-Horizontal_VT-ARC.png", x=5, y=5, w=50)
            self.image(f"{figs}/SSC logo.png", x=170, y=3, w=12)
            self.image(f"{figs}/TAB_LAB_invert_1.png", x=185, y=3, w=20)
            self.image(f"{figs}/sean_explains.png", x=10, y=20, w=20)
            self.image(f"{figs}/TicTac.png",  x=195, y=285, w=5)
    ...
    State_label = ["NORAD ID","MD P-Score",
                "Pos. Error Norm", "Vel. Error Norm","Total Bias", "NEES P-Score"]
    ...
    for x in range(number):
        ...
        pdf.cell(col_width, row_height, str(states_current["Position Error Norm"]), border=1, align="C")
        pdf.cell(col_width, row_height, str(states_current["Velocity Error Norm"]), border=1, align="C")
