...
from uctbenchmark.src.libraries.basicScoringFunction import basicScoring
import uctbenchmark.src.libraries.config as con
import os
import json
...
def batchPull(code, start_epoch, end_epoch, udl_token):
    ...
    #Saving call results to Raw Directory
    filename = f'{str(code)}_0_0_4_{str(sensor_type)}_{str(start_epoch)}_{str(end_epoch)}'.replace(':','_').replace(' ','_').replace('-','_')
    with open(f'{filename}.json', 'a') as data_file:
        data_compiled.to_json(data_file, orient='records', indent=4)
...
def thresholdCheck(batch_temp, thresh_des, code, sat_data_f):
    '''
    ...
    thresh_curr, _, _ = basicScoring(code, batch_temp, satData = pd.read_csv(sat_data_f))
...
def slide(sub_batch, window_size, code, sat_data_f):
    '''
    ...
    while i >= 0 and tc + window_size <= t_max:
        ...
        # Score the bin
        score_temp, orbElems_temp, metadata_temp = basicScoring(code, bin, satData = pd.read_csv(sat_data_f))
...
def bisect(batch, window_size, thresh_des, code):
    ...
    def recurse(sub_batch):
        ...
        check = thresholdCheck(sub_batch, thresh_des, code, sat_data_f)
        ...
        # Bin is valid but is minimum size and must be swept
            ...
            bin_temp, score_temp, orbElems_temp, metadata_temp = slide(sub_batch, window_size, code, sat_data_f)
...
def windowCheck(window_size, batch_size, code, start_epoch, end_epoch, udl_token):
    ...
    for i in range(len(threshold_scores)):
        ...
        if thresholdCheck(data_sorted, thresh_des, code, sat_data_f):
...
def windowMain(codes, udl_token, raw_dir):
    '''
    ...
    for code in codes:
        ...
        # Creating directory for UDL response under search code and current datetime
        dir_name = f"{code}_0_0_4_{str(dt.datetime.now()).replace("-","_").replace(":","_").replace(".","")}"
        code_full_path = os.path.join(os.getcwd(),dir_name)
        if os.path.isdir(code_full_path):
            print("This data looks familiar...")
        else:
            os.mkdir(code_full_path)
            
        os.chdir(code_full_path)
