...
import requests, base64, warnings, pandas as pd, re, datetime, asyncio, aiohttp, time, numpy as np, json
import math
import os 
import jdk4py
...
setup_orekit_data(from_pip_library=True) #Loading orekit data
...
def UDLQuery(token, service, params, count=False, history=False):
    ...
    if resp.status_code != 200:
        if resp.status_code == 400:
            ...
            raise requests.exceptions.HTTPError(resp, "Query failed due to internal error; if UDL isn't down, likely a time-out for excessive data request.")
        else:
...
def parseTLE(line1, line2):
    ...
    elset = {
        ...
        'n_ddot': float(re.sub(r'^([+-]?)(\d{5})([+-]\d)$', r'\g<1>0.\g<2>e\g<3>', lines[5])) if lines[5] != '00000-0' else 0.0,
        'B_star': float(re.sub(r'^([+-]?)(\d{5})([+-]\d)$', r'\g<1>0.\g<2>e\g<3>', lines[6])) if lines[6] != '00000-0' else 0.0,
...
def discoswebQuery(token, params, data='objects', version=2):
    ...
    # Error handling
    if not (isinstance(token,str) and isinstance(data,str) and isinstance(params,str) and isinstance(version,int)):
        raise TypeError(f"Expected (string, string, string, int), got ({type(token).__name__}, {type(params).__name__}, {type(data).__name__}, {type(version).__name__}) instead.")
    ...
    if resp.status_code != 200:
        if resp.status_code == 429:
            ...
            raise requests.exceptions.HTTPError(resp, "Query failed for unknown reason ("+str(resp.status_code)+"); double-check login info and query parameters.")
...
def celestrakQuery(params, table='gp'):
    ...
    if resp.status_code != 200:
        raise requests.exceptions.HTTPError(resp, "Query failed for unknown reason ("+str(resp.status_code)+"); double-check query parameters.")
    return pd.DataFrame(resp.json())
...
def generateDataset(UDL_token, ESA_token, satIDs,timeframe,timeunit,dt=0.1,max_datapoints=0,end_time='now'):
    """
    ...
    if end_time != 'now':
        ...
        sweep_time = datetimeToUDL(end_time - pd.Timedelta(**{timeunit: timeframe})) + '..' + datetimeToUDL(end_time)
    else:
    ...
    if state_truth_data['satNo'].nunique() < len(state_truth_data):
        ...
        df['has_cov'] = np.any(
            [df[col].notna() if col in df.columns else pd.Series(False, index=df.index) for col in cov_sources],
            axis=0
        )
        ...
        if not df_with_cov.empty:
            ...
            state_truth_data = (
                df_with_cov_sorted
                .groupby('satNo', group_keys=False)
                .head(1)
    ...
    state_truth_data = state_truth_data.fillna({
        'mass': 0,
        'crossSection': 0
    })
    ...
    if end_time == 'now':
        elset_truth_data = UDLQuery(UDL_token, "elset/current", {
    ...
    elset_truth_data["elset"] = elset_truth_data.apply(lambda row: parseTLE(row['line1'], row['line2']), axis=1)
    ...
    dataset = dataset.drop(columns=['satNo', 'idOnOrbit', "origObjectId", "rawFileURI", "createdAt", "trackId", "has_cov"], errors='ignore')
...
def pullStates(UDL_token,satIDs,timeframe,timeunit,dt=0.1,end_time='now'):
    ...
    # Use "current" call if allowed, else grab individuals
    if end_time == 'now':
        elset_data = UDLQuery(UDL_token, "elset/current", {
            "satNo": ",".join(map(str, satIDs)),
        })
    else:
        params_list = [{
...
def saveDataset(ref_obs,ref_track,ref_sv,ref_elset,output_data):
    '''
    ...
    orbit_data['cov'] = [
        str(arr).replace("[","").replace("]","").split(", ") for arr in orbit_data['cov'].values
    ]
...
def loadDataset(input_path):
    ...
    obs_data['obTime'] = pd.to_datetime(obs_data['obTime'], format='ISO8601')
    ...
    # Reconstruct ref_sv
    ref_sv = reference[['satNo', 'xpos', 'ypos', 'zpos', 'xvel', 'yvel', 'zvel', 'epoch', 'cov', 'mass', 'crossSection', 'dragCoeff', 'solarRadPressCoeff']].copy()
    ref_sv['epoch'] = pd.to_datetime(ref_sv['epoch'])
    ref_sv['cov_matrix'] = ref_sv['cov'].apply(lambda x: np.array([x], dtype=np.float64))
