...
def  monteCarloPropagator(stateVector,covariance,initialEpoch,finalEpoch,N=0,satelliteParameters = [1000,13.873,0,0]):
    ...
    covariance: 6x6 numpy array, value is arbitrary if N=0
    ...
    finalStateVector = np.concatenate((np.array(pvT.getPosition().toArray()),np.array(pvT.getVelocity().toArray())))
    ...
    if N>1:
        ...
        for state_vector in MCpoints:
            try: # Catch inside ellipsoid errors (satellite decays to inside earth radius)
                ...
                initial_position = Vector3D(float(state_vector[0]), float(state_vector[1]), float(state_vector[2]))
                initial_velocity = Vector3D(float(state_vector[3]), float(state_vector[4]), float(state_vector[5]))
                initial_pv = PVCoordinates(initial_position, initial_velocity)
                initial_orbit = CartesianOrbit(initial_pv, inertial_frame, initialAbsDate, mu)
                initial_state = SpacecraftState(initial_orbit,float(satelliteMass))
                propagator_num.setInitialState(initial_state)
                ...
                MCstates.append(np.concatenate((np.array(pvT.getPosition().toArray()),np.array(pvT.getVelocity().toArray()))))
            except orekit.JavaError as e: # Skip failed point, continue with rest of sample point
                failedProps +=1
                continue
        #print(failedProps)
...
def  ephemerisPropagator(stateVector,initialEpoch,finalEpoch,satelliteParameters = [1000,13.873,0,0]):
    ...
    solarCoefficient = config.solarRadPresCoef # Default solar radiation pressure coefficient for boxwing
    ...
    if t_obs_after:
        ...
        for t in t_obs_after:
            ...
            stateVector = np.concatenate((np.array(pv.getPosition().toArray()), np.array(pv.getVelocity().toArray())))
            if convertFlag:
...
def TLEpropagator(input1, input2, finalEpoch):
    ...
    convertFlag = False
    if isinstance(input1, str) == False:
        stateVector = input1
        ...
        orbit = CartesianOrbit(pvCoordinates, FramesFactory.getEME2000(), initialEpoch, Constants.WGS84_EARTH_MU)
    ...
    if t_obs_after:
        ...
        for t in t_obs_after:
            ...
            stateVector = 1e-3 * np.concatenate((np.array(pv.getPosition().toArray()), np.array(pv.getVelocity().toArray())))
    ...
    return t_before_line1 + t_after_line1, t_before_line2 + t_after_line2, t_before_state_list + t_after_state_list       
...
def orbit2OE(input1, input2):
    ...
    if isinstance(input1,str):
        ...
        kep_orbit = KeplerianOrbit(pvCoord, FramesFactory.getEME2000(), initialEpoch, Constants.WGS84_EARTH_MU)
    ...
    return {
        ...
        'Argument of Perigee': float(np.degrees(kep_orbit.getPerigeeArgument())),          # Argument of perigee (degrees)
...
if __name__ =="__main__":
    ...
    if testCase==2:
        ...
        finalEpoch = [datetime(2025,6,8,9,10,17,341000),datetime(2025,6,8,9,10,17,341500),datetime(2025,6,8,9,10,17,342500),datetime(2025,6,8,9,10,17,343000),datetime(2025,6,8,9,10,17,342000)]

        initialState = 1e-3*np.array([-7365971.28111,-1331399.7084,1514249.14205,1976.71138257,-5225.284848027,4472.66176583])
        initialCov =  1e9*np.array([
            [2.79656e-12, -1.66714e-12, 1.3131e-13, 4.48328e-13, -5.45997e-9, -4.98079e-12],   # dx uncertainty (e.g., 10 m^2 variance)
            [-1.66714e-12, 1.15089e-12, 1.37069e-13, -2.02594e-15, 4.86833e-10, 7.70187e-13],   # dy
            [1.3131e-13, 1.37069e-13, 4.68529e-13, 4.36053e-13, -4.55737e-9, -3.56265e-12],   # dz
            [4.48328e-13, -2.02594e-15, 4.36053e-13, 5.70924e-13, -5.5708e-9, -4.50443e-12],  # dvx uncertainty (e.g., 0.001 (m/s)^2 variance)
            [-5.45997e-9, 4.86833e-10, -4.55737e-9, -5.5708e-9, 0.0000647153, 5.25295e-8],  # dvy
            [-4.98079e-12, 7.70187e-13, -3.56265e-12, -4.50443e-12, 5.25295e-8, 4.28811e-11]   # dvz
        ])
    ...
    if testCase == 3:
        ...
        initialState=np.array([-41105.92780995852e3, -9050.561409355181e3, -1809.49929235732e3,0.671176665193438e3,-2.910029484561473e3, -0.741988755546521e3])
        initialCov =  1e9*np.array([
            [2.79656e-12, -1.66714e-12, 1.3131e-13, 4.48328e-13, -5.45997e-9, -4.98079e-12],   # dx uncertainty (e.g., 10 m^2 variance)
            [-1.66714e-12, 1.15089e-12, 1.37069e-13, -2.02594e-15, 4.86833e-10, 7.70187e-13],   # dy
            [1.3131e-13, 1.37069e-13, 4.68529e-13, 4.36053e-13, -4.55737e-9, -3.56265e-12],   # dz
    ...
    if testCase == 'tle':   
        initialEpoch = datetime(2025,6,8,8,10,17,342000)
        finalEpoch = [datetime(2025,6,8,9,10,17,342000),datetime(2025,6,8,10,10,17,342000)]
    ...
    if testCase == 'SV-tle':   
        initialEpoch = datetime(2025,6,8,8,10,17,342000)
        finalEpoch = [datetime(2025,6,8,9,10,17,342000),datetime(2025,6,8,10,10,17,342000)]
        initialState = np.array([-7365971.28111,-1331399.7084,1514249.14205,1976.71138257,-5225.284848027,4472.66176583])
